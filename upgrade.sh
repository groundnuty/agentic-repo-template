...
#   - v0.5.0 retires CLAUDE.md: your own text in CLAUDE.md, .claude/CLAUDE.md
#     and rules/project-conventions.md is MOVED into AGENTS.md (below its
#     template-managed region) and each file removed only after the move is
#     verified; pure-template sections are dropped. Root AGENTS.md and CLAUDE.md
#     are copied into the backup first. A migrated rule is deleted only when it
#     was placed in AGENTS.md and carries no edits of yours (v0.4.12).
...
if [ "$DRY_RUN" -eq 1 ]; then
  ...
  echo "[dry-run] would overlay template-owned files in place (custom files untouched); apply removed-files.txt;"
  echo "[dry-run] regenerate root files: $ROOT_FILES; refresh AGENTS.md's managed region; move your text out of"
  echo "[dry-run] CLAUDE.md, .claude/CLAUDE.md, rules/project-conventions.md into AGENTS.md (v0.5.0), verified, then remove them"
  echo "[dry-run] would preserve:   settings.local.json, CLAUDE.local.md, audit.log, session-reports/"
...
#     (a v0.5.0 $WORK generates neither; the guard stays for safety — those
#     files are retired into AGENTS.md by the dedicated step below)
...
# --- AGENTS.md migration (v0.4.0; the only instruction file since v0.5.0) ---
# Every harness reads AGENTS.md natively (Claude Code 2.1.277+). $WORK already
# holds a freshly composed AGENTS.md for this profile.
#
# ONLY the fenced managed region is replaced; everything the user wrote outside
# the fences survives (D49). A hand-written AGENTS.md with no fences gets the
# managed region PREPENDED, its own text untouched below (v0.5.0 — before that
# it was left alone, and v0.4.0–v0.4.11 then lost the rules, D58). A symlinked
# AGENTS.md is never written through.
...
migrate_agents_md() {
  ...
  if [ -L AGENTS.md ]; then
    # A symlink (e.g. one AGENTS.md shared by several repos). Writing through it
    # would change every repo sharing it; replacing it would break the sharing.
    # Leave it alone: without a placed corpus, apply_migrations deletes nothing.
    AGENTS_NOTE="  AGENTS.md:   a symlink -> $(readlink AGENTS.md) — left alone; the rules stay in
               .claude/rules/ and CLAUDE.md is not retired. Merge by hand if you want them in it."
    return 0
  fi
  if [ ! -f AGENTS.md ]; then
    ...
    # Your own AGENTS.md, written before the template managed one. v0.5.0 puts the
    # managed region ABOVE your text and leaves every byte of yours in place: the
    # corpus has to reach AGENTS.md now that there is no CLAUDE.md to carry it.
    # (v0.4.x left this file alone — and v0.4.0–v0.4.11 then lost the rules, D58.)
    { printf '%s\n\n' "$managed"; cat AGENTS.md; } > AGENTS.md.new && mv AGENTS.md.new AGENTS.md
    AGENTS_NOTE="  AGENTS.md:   yours — the template-managed region was added ABOVE your text,
               which is unchanged below it (original: $BACKUP/AGENTS.md.orig)."
  fi

}
# The .claude/ backup does not cover root files; v0.5.0 modifies AGENTS.md and
# retires a root CLAUDE.md, so copy both first.
[ -f AGENTS.md ] && cp -p AGENTS.md "$BACKUP/AGENTS.md.orig" 2>/dev/null
[ -f CLAUDE.md ] && cp -p CLAUDE.md "$BACKUP/CLAUDE.md.orig" 2>/dev/null
AGENTS_NOTE=""
migrate_agents_md
...
version_ge_u() { [ "$(printf '%s\n%s\n' "$1" "$2" | sort -t. -k1,1n -k2,2n -k3,3n | head -1)" = "$2" ]; }

# --- retire CLAUDE.md (v0.5.0) -----------------------------------------------
# Move the consumer's own text out of CLAUDE.md, .claude/CLAUDE.md and
# .claude/rules/project-conventions.md into AGENTS.md (below the managed region);
# pure-template sections are dropped; a file is deleted only after its moved
# lines are verified in AGENTS.md. The helper and its data run from the template
# source — never from the consumer tree.
RETIRE_REPORT=""
if [ -f AGENTS.md ] && [ ! -L AGENTS.md ]; then
  RETIRE_REPORT="$(mktemp)"
  for f in CLAUDE.md .claude/CLAUDE.md .claude/rules/project-conventions.md; do
    [ -e "$f" ] || continue
    if command -v python3 >/dev/null 2>&1; then
      python3 "$SRC/.claude/migrate-instructions.py" --agents AGENTS.md \
        --template-lines "$SRC/.claude/template-lines.txt" "$f" >> "$RETIRE_REPORT" 2>&1 || true
    else
      printf 'KEPT\t%s\tpython3 not found — nothing moved\n' "$f" >> "$RETIRE_REPORT"
    fi
  done
fi
...
echo "  Untouched: settings.local.json, audit.log, session-reports/, CLAUDE.local.md, and every"
echo "             custom file you added under .claude/"
echo "  Merged:    template-owned files updated in place"
...
if [ -n "$MIGRATED_LIST" ]; then
  ...
  echo "    Claude Code (2.1.277+), Codex and OpenCode all read AGENTS.md natively."
  echo "    Nothing was lost: each was deleted only after its text was verified there."
...
if [ -n "$RETIRE_REPORT" ] && [ -s "$RETIRE_REPORT" ]; then
  echo
  echo "  CLAUDE.md RETIRED (v0.5.0) — AGENTS.md is now the only instruction file:"
  awk -F'\t' '
    $1=="MOVED"         {printf "    %-40s your text moved into AGENTS.md (%s lines), file removed\n", $2, $3}
    $1=="TEMPLATE-ONLY" {printf "    %-40s template boilerplate only, removed\n", $2}
    $1=="LINK-REMOVED"  {printf "    %-40s symlink to AGENTS.md, removed (the text is in AGENTS.md)\n", $2}
    $1=="KEPT"          {printf "    %-40s KEPT — %s\n", $2, $3}
    $1=="UNRESOLVED"    {printf "    %-40s note: link %s does not resolve after the move\n", $2, $3}' "$RETIRE_REPORT"
  echo "    Originals: $BACKUP/ (CLAUDE.md.orig, and .claude/CLAUDE.md inside the backup)."
  rm -f "$RETIRE_REPORT"
fi
# Codex reads only the first project_doc_max_bytes of AGENTS.md (default 32768,
# codex-rs/config/defaults.toml) and truncates the rest with no visible warning.
# Your own text sits BELOW the managed region, so it is what gets cut.
agents_size_warning() {
  local b; [ -f AGENTS.md ] || return 0
  b=$(wc -c < AGENTS.md | tr -d ' ')
  [ "$b" -gt 32768 ] || return 0
  echo
  echo "  WARNING: AGENTS.md is $((b / 1024)) KiB. Codex reads only the first 32 KiB by default and"
  echo "           silently drops the rest — your own text, which sits below the managed region."
  echo "           Raise it in ~/.codex/config.toml:  project_doc_max_bytes = 262144"
}
agents_size_warning

# Text moved out of an old .claude/CLAUDE.md can still list rule files that no
# longer exist (the "Rules applied" list). It is your text, so it is moved, not
# edited — but say so, because a stale list is what once sent an agent hunting
# for "lost" rules (v0.4.8 defect report).
stale="$(awk -v b="$AGENTS_BEGIN" -v e="$AGENTS_END" '
  index($0, b) {m=1; next} index($0, e) {m=0; next}
  !m && /`(autonomous-work|pr-discipline|writing-quality|citation-discipline|knowledge-work-structure|session-logging|prompt-shaping|reading-before-editing)\.md`/ {print "    " $0}
' AGENTS.md 2>/dev/null | head -5)"
if [ -n "$stale" ]; then
  echo
  echo "  STALE DOC: your part of AGENTS.md still names rule files that no longer exist"
  echo "  (their text is in the managed region above). Delete these lines when convenient:"
  printf '%s\n' "$stale"
fi
v_now="$(cc_version_u || true)"
if [ -z "$v_now" ] || ! version_ge_u "$v_now" "2.1.277"; then
  echo
  echo "  WARNING: Claude Code ${v_now:-not found} reads AGENTS.md natively only from 2.1.277. With no"
  echo "           CLAUDE.md, an older Claude Code sees NO project instructions. Upgrade Claude Code."
fi
if [ -f CLAUDE.local.md ]; then
  echo
  echo "  WARNING: CLAUDE.local.md is present (left untouched — it is private). It stops Claude Code"
