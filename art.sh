...
#   art install-wrappers          install `claudet` / `codext` / `opencodet` — the same behaviour,
#                                 typed like the tool itself. --uninstall removes them.
...
# Install `<tool>t` commands: claudet, codext, opencodet. Deliberately NOT a
# shadow of the real binary — that would need PATH surgery and would change what
# `claude -p` means for every script on the machine. A distinct name clashes with
# nothing, needs no PATH reordering, and says what it does.
WRAPPER_TEMPLATE='#!/usr/bin/env bash
# @TOOL@t - run @TOOL@ in a tmux session named after the working directory.
# Created once, reattached every time after. Generated by `art install-wrappers`.
#
# Every argument passes through untouched. It steps aside and execs @TOOL@
# directly whenever tmux would be wrong: no TTY (scripts, pipes, CI), already
# inside tmux, a -p/--print run, a non-interactive subcommand, or ART_NO_TMUX=1.
set -euo pipefail
REAL="@REAL@"
[ -x "$REAL" ] || REAL="$(command -v @TOOL@ 2>/dev/null || true)"
[ -n "$REAL" ] || { echo "@TOOL@t: cannot find @TOOL@" >&2; exit 127; }

[ -n "${ART_NO_TMUX:-}" ] && exec "$REAL" "$@"
[ -n "${TMUX:-}" ] && exec "$REAL" "$@"
command -v tmux >/dev/null 2>&1 || exec "$REAL" "$@"
{ [ -t 0 ] && [ -t 1 ]; } || exec "$REAL" "$@"

case "${1:-}" in
  exec|run|mcp|plugin|doctor|update|login|logout|auth|completion|apply|resume|serve|sandbox|debug|review|app|mcp-server|app-server|remote-control|--version|-v|--help|-h)
    exec "$REAL" "$@" ;;
esac
for arg in "$@"; do
  case "$arg" in -p|--print) exec "$REAL" "$@" ;; esac
done
...
exec tmux new-session -s "$name" -c "$dir" "$REAL" "$@"
'

cmd_install_wrappers() {
  local bindir="$HOME/.local/bin" uninstall=0 tool real wrapper made=0
  while [ $# -gt 0 ]; do
    case "$1" in
      --dir) bindir="${2:?--dir needs a path}"; shift 2 ;;
      --dir=*) bindir="${1#*=}"; shift ;;
  ...
  for tool in claude codex opencode; do
    wrapper="$bindir/${tool}t"
    if [ "$uninstall" = "1" ]; then
      [ -f "$wrapper" ] && { rm -f "$wrapper"; echo "  removed $wrapper"; made=$((made+1)); }
      continue
    fi
    real="$(command -v "$tool" 2>/dev/null || true)"
    [ -n "$real" ] || { echo "  skip ${tool}t - '$tool' is not on PATH"; continue; }
    [ "$real" = "$wrapper" ] && { echo "  skip ${tool}t - would point at itself"; continue; }
    printf '%s' "$WRAPPER_TEMPLATE" | sed -e "s|@TOOL@|$tool|g" -e "s|@REAL@|$real|g" > "$wrapper"
    chmod +x "$wrapper"
    echo "  ${tool}t -> $real"
    made=$((made+1))
  done

  [ "$made" -gt 0 ] || { echo "nothing to do"; return 0; }
  [ "$uninstall" = "1" ] && { echo; echo "wrappers removed."; return 0; }
  echo
  case ":$PATH:" in
    *":$bindir:"*) echo "$bindir is on your PATH - try: claudet" ;;
    *) echo "NOTE: $bindir is not on your PATH. Add it, then restart your shell:"
...
case "${1:-}" in
  ...
  tmux)     shift; cmd_tmux "$@" ;;
  install-wrappers) shift; cmd_install_wrappers "$@" ;;
